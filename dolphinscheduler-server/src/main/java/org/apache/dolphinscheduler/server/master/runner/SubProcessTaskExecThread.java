...
public class SubProcessTaskExecThread extends MasterBaseTaskExecThread {
    ...
    private void waitTaskQuit() throws InterruptedException {
        ...
        if (fakeRun) {
            logger.info("subflow task :{} id:{}, process id:{}, as fake-run",
                    this.taskInstance.getName(), this.taskInstance.getId(), this.processInstance.getId());
            return;
        }
    ...
    private void stopSubProcess() {
        ExecutionStatus subProcessInstanceState = subProcessInstance.getState();
        if (subProcessInstanceState == ExecutionStatus.STOP
                || subProcessInstanceState == ExecutionStatus.READY_STOP) {
            return;
        }
    ...
    private void pauseSubProcess() {
        ExecutionStatus subProcessInstanceState = subProcessInstance.getState();
        if (subProcessInstanceState == ExecutionStatus.PAUSE
                || subProcessInstanceState == ExecutionStatus.READY_PAUSE) {
            return;
        }
